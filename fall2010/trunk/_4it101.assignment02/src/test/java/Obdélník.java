...
public class Obdélník
        implements Cloneable, ITvar, IPosuvný, INafukovací, IHýbací
{
...
//== KONSTRUKTORY A TOVÁRNÍ METODY =============================================
    ...
    public ITvar kopie()
    {
        if (this instanceof ITvar) {
            return (ITvar)new Obdélník(xPos, yPos, šířka, výška, barva);
        } else {
            //Pomocná třída zabezpečující správnou funkci i v případě,
            //kdy třída nebude implementovat rozhraní ITvar
            class TO extends Obdélník implements ITvar {
                TO(int x, int y, int s, int v, Barva b) {
                    super(x, y, s, v, b);
                }
            }
            return new TO(xPos, yPos, šířka, výška, barva);
        }
    }
